...
int try_sigsegv()
{
    ...
    long size = 1024UL*1024;//*1024*200; //sizeof(int);
    ...
    /////////////////////////////////////////
    int teno;
    int fd = open("/tmp/myfile", O_RDWR|O_DIRECT, 0644);
    if( fd<=0 ){
    ...
    /**
    At write(): when read from the protected area, read failed, and errno is EFAULT (14).
    at read():  when write to the protected area, write failed, and errno is EFAULT (14).
    **/
    i = 0;    
    do{
        teno = 0;
        i = pread(fd, p1, 4096, 0); 
        if( i!=4096 ){
            teno = errno;
            perror("fail to write:: ");
            if( teno == EFAULT ){
                /*   on read():  a new version of data is going to be written to vaddr,
                    here we shall discard any slabcache associated with this vaddr.
                */
                mprotect ((void *)p1, 4096, PROT_WRITE); 
                // put p1 into pg-buf since it has been instantiated.
            }
            //printf("errno=%d: %s\n", teno, strerror(teno) );
        } 
        else if( teno != 0 ){
            //mprotect ((void *)p1, 4096, PROT_NONE);
        }
    }
    while( teno==EFAULT );

    mprotect ((void *)p1, size, PROT_NONE); 

    do{
        teno = 0;
        i = pwrite(fd, p1, 4096, 1024); 
        if( i!=4096 ){
            teno = errno;
            perror("fail to write:: ");
            if( teno == EFAULT ){
                /*   on write():  shall get the latesy data version for vaddr from slabcache,
                    shall find the slab-cache obj, copy it to vaddr:: 
                */
                mprotect ((void *)p1, 4096, PROT_READ); 
                // put p1 into pg-buf since it has been instantiated.
            }
            //printf("errno=%d: %s\n", teno, strerror(teno) );
        } 
