...
struct PageAllocationTableNode {
  PageAllocationTableNode() : number_free_pages_(0), number_total_pages_(0),
  number_used_pages_(0), entries_(NULL) {}

  ~PageAllocationTableNode() {}

  bool Init(uint64_t* entries, uint64_t number_entries, uint64_t
      max_per_entry_pages, uint64_t total_pages);
  ...
  bool GetPages(uint64_t free_pages_wanted, std::vector<uint64_t>*
...
//
// Corresponding to the 3-level struct, a page number is divided up to
// 3 fields from MSB to LSB:  X bits (PGD), Y bits (PMD), and Z bits (which bit
// in the bitmap).
//
